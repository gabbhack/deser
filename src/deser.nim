...
`serialize` and `deserialize` are functions that tell the parser how to serialize or deserialize your type.
Deser generates them automatically when you use the Use [makeSerializable](deser/ser/make.html#makeSerializable.m,varargs[typedesc],static[bool]) and [makeDeserializable](deser/des/make.html#makeDeserializable.m,varargs[typedesc],static[bool]) macros.

To get a better understanding of the serialization process and how macros work, let's write the serialization functions ourselves.

.. Note:: If you just want to see the code generated by the macro, use the `-d:debugMakeSerializable` or/and `-d:debugMakeDeserializable` flags.

## Serialize
Let's start with the `serialize` function. It takes two arguments: the object to be serialized and the serializer.

```nim
# Type that we want to serialize
type Message = object
  id: int
  text: string
  created: int

proc serialize(obj: Message, serializer: var auto) =
  discard
```
