...
proc addSkipSerializeIf(x: var seq[FieldDescription], skipSerializeIf: NimNode) =
  for i in countup(0, high(x)):
...
proc renamed*(field: FieldDescription, op: Operation = Des): NimNode =
  let rename = field.pragmas.findPragma(bindSym"rename")
  let renameAll = field.renameAll
  if rename != nil:
    case op
    of Des:
      if rename[1].strVal.len > 0 and rename[2].strVal.len == 0:
        return newLit rename[1].strVal
      elif rename[2].strVal.len > 0:
        return newLit rename[2].strVal
      else:
        return newLit field.name.strVal
    of Ser:
      if rename[1].strVal.len > 0:
        return newLit rename[1].strVal
      else:
        return newLit field.name.strVal
  elif renameAll != nil:
    case op
    of Des:
      if renameAll[1].strVal != "rkNothing" and renameAll[2].strVal == "rkNothing":
        return newLit renamer(field.name.strVal, renameAll[1].strVal)
      else:
        return newLit renamer(field.name.strVal, renameAll[2].strVal)
    of Ser:
      return newLit renamer(field.name.strVal, renameAll[1].strVal)
  else:
    return newLit field.name.strVal

proc getCases*(field: FieldDescription): seq[tuple[branch: NimNode, fields: seq[FieldDescription]]] =
